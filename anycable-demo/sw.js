...
const installApp = async () => {
  ...
  self.anycableFetch = {
    perform: function (url, publication) {
      fetch(url, {
        method: "POST",
        body: publication,
      })
...
const respondWithRails = async (event) => {
  ...
  const railsURL = event.request.url;
  const railsHeaders = {};
  ...
  try {
    ...
    railsHeaders["CONTENT_TYPE"] = railsHeaders["HTTP_CONTENT_TYPE"];

    let input = "nil";

    if (!(event.request.method === "GET" || event.request.method === "HEAD")) {
      let rawInput = await event.request.text();
